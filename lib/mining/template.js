...
BlockTemplate.prototype.getCoinstake = function getCoinstake(nTime, coin, fees) {
  let coinstakeTx = new TX();
  coinstakeTx.txTime = nTime;
  coinstakeTx.outputs[0] = new Output();
  coinstakeTx.outputs[1] = new Output();
  if(coin.script.isPubkeyhash()){
    coinstakeTx.outputs[1].script = new Script();
    let code = [];
    let op = new Opcode(0xac, coin.script.getPubKey());
    code.push(op);
    coinstakeTx.outputs[1].script.fromCode(code);
  } else if(coin.script.isPubkey()){
    coinstakeTx.outputs[1].script = coin.script;
  }
  coinstakeTx.outputs[1].value = coin.value + consensus.getReward() + fees;
  coinstakeTx.inputs[0] = new Input();
  coinstakeTx.inputs[0].fromCoin(coin);
  return coinstakeTx;
};
...
BlockTemplate.prototype.commitCnTxTime = function commitCnTxTime(nTime, coin) {
  let block = new Block();
  let tx;
  ...
  block.txs[1] = this.getCoinstake(nTime, coin, fees);
  // add block signature, also add signatures to Block.toRaw
  return block;
};
