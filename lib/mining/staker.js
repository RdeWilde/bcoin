...
const STDStaker = require('./stdstaker');
const BlockEntry = BlockTemplate.BlockEntry;

/**
 * A bitcoin miner and block generator.
 * @alias module:mining.Staker
 * @constructor
 * @param {Object} options
 */

function Staker(options) {
  if (!(this instanceof Staker))
    return new Staker(options);

  AsyncObject.call(this);

  this.options = new StakerOptions(options);
  ...
  this.wallet = null;
  this.account = null;
  this.cpu = new STDStaker(this);

  this.init();
}

util.inherits(Staker, AsyncObject);

/**
 * Open the miner, wait for the chain and mempool to load.
 * @method
 * @alias module:mining.Staker#open
 * @returns {Promise}
 */

Staker.prototype.init = function init() {
...
/**
 * Open the miner, wait for the chain, wallet, account and mempool to load.
 * @method
 * @alias module:mining.Staker#open
 * @returns {Promise}
 */

Staker.prototype._open = async function open() {
 ...
 * Close the miner.
 * @method
 * @alias module:mining.Staker#close
 * @returns {Promise}
 */

Staker.prototype._close = async function close() {
...
Staker.prototype.createBlock = async function createBlock(tip, address) {
...
Staker.prototype._createBlock = async function createBlock(tip, address) {
  ...
  if (this.options.preverify) {
    ...
    try {
      ...
      if (e.type === 'VerifyError') {
        this.logger.warning('Staker created invalid block!');
        this.logger.error(e);
        throw new Error('BUG: Staker created invalid block.');
      }
      throw e;
    }
...
Staker.prototype.updateTime = function updateTime(attempt) {
 ...
 * @param {ChainEntry?} tip
 * @param {Address?} address
 * @returns {Promise} Returns {@link STDJob}.
 */

Staker.prototype.createJob = function createJob(tip, address) {
...
Staker.prototype.mineBlock = function mineBlock(tip, address) {
...
Staker.prototype.addAddress = function addAddress(address) {
...
Staker.prototype.getAddress = function getAddress() {
  if (this.addresses.length === 0)
    return new Address();
...
Staker.prototype.assemble = function assemble(attempt) {
...
/**
 * StakerOptions
 * @alias module:mining.StakerOptions
 * @constructor
 * @param {Object}
 */

function StakerOptions(options) {
  if (!(this instanceof StakerOptions))
    return new StakerOptions(options);
 ...
 * @private
 * @param {Object} options
 * @returns {StakerOptions}
 */

StakerOptions.prototype.fromOptions = function fromOptions(options) {
  assert(options, 'Staker requires options.');
  assert(options.chain && typeof options.chain === 'object',
    'Staker requires a blockchain.');

  assert(options.walletdb && typeof options.walletdb === 'object',
    'Staker requires a walletdb.');
...
StakerOptions.fromOptions = function fromOptions(options) {
  return new StakerOptions().fromOptions(options);
};
