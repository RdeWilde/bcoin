...
/**
 * Do checkBlockSignature
 * @returns {Boolean}
 */

Block.prototype.checkBlockSignature = function checkBlockSignature() {
  if(!this.stake && this.signature.length === 0)
    return true;

  if(this.signature.length === 0)
    return false;

  let txout = this.txs[1].outputs[1];
  return secp256k1.verify(this.hash(), this.signature, txout.script.getPubKey());
};
...
Block.prototype.checkBody = function checkBody() {
  ...
  if(this.ts > consensus.futureDriftV2(util.now()))
    return [false, 'bad-the-future', 100];
  ...
  if (this.stake) {
    ...
    if (this.txs[0].outputs.length != 1 || !this.txs[0].outputs[0].isEmpty())
      return [false, 'bad-cb-not-empty', 100];

    // Second transaction must be coinstake, the rest must not be
    if (this.txs.length < 2 || !this.txs[1].isCoinstake())
      return [false, 'bad-cs-second', 100];

    for (var i = 2; i < this.txs.length; i++)
      if (this.txs[i].isCoinstake())
        return [false, 'bad-cs-multiple', 100];

    // Check proof-of-stake block signature
    if (!this.checkBlockSignature())
      return [false, 'bad-signature', 100];
...
Block.prototype.getClaimed = function getClaimed() {
  ...
  assert(this.txs[1].isCoinstake());
  return this.txs[1].getOutputValue();
};
...
Block.prototype.fromReader = function fromReader(br) {
  ...
  this.stake = this.txs.length > 1 && this.txs[1].isCoinstake();
  if(this.stake){
