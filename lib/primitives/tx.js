...
TX.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.txTime != null) {
    assert(util.isUInt32(options.txTime), 'TxTime must be a uint32.');
    this.txTime = options.txTime;
  }
...
TX.prototype.signatureHash = function signatureHash(index, prev, value, type, version, txTime) {
  ...
  if (version === 0)
    return this.signatureHashV0(index, prev, type, txTime);

  // Segwit sighashing
  if (version === 1)
    return this.signatureHashV1(index, prev, value, type, txTime);
...
TX.prototype.signatureHashV0 = function signatureHashV0(index, prev, type, txTime) {
  let size, bw;
...
TX.prototype.isCoinstake = function isCoinstake() {
...
TX.prototype.checkInputs = function checkInputs(view, height) {
  ...
  if (!this.isCoinstake() && total < value)
    return [-1, 'bad-txns-in-belowout', 100];

  fee = total - value;

  if (!this.isCoinstake() && fee < 0)
...
TX.prototype.fromJSON = function fromJSON(json) {
  ...
  assert(util.isUInt32(json.txTime), 'TxTime must be a uint32.');
...
TX.isWitness = function isWitness(br) {
  return false;
  // disabled, segwit tx is detected in Block 306(false-positive)
  // if (br.left() < 6)
  //   return false;
  //
