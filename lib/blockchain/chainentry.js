...
function ChainEntry(chain, options, prev) {
  ...
  this.height = -1;
  this.chainwork = null;
  this.stakeModifier = encoding.NULL_HASH;
  this.stake = false;
...
ChainEntry.prototype.getStakeModifier = function getStakeModifier(prev) {
  let stake = encoding.NULL_HASH;

  if (!prev)
    return stake;

  return prev.stakeModifier;
};
...
ChainEntry.prototype.fromBlock = function fromBlock(block, prev) {
  ...
  this.chainwork = this.getChainwork(prev);
  this.stakeModifier = this.getStakeModifier(prev);
  this.stake = block.stake;
  return this;
};
...
ChainEntry.prototype.toRaw = function toRaw() {
  ...
  bw.writeHash(this.stakeModifier);
  bw.writeU8(this.stake? 1 : 0);
...
ChainEntry.prototype.fromRaw = function fromRaw(data) {
  ...
  this.stakeModifier = br.readHash('hex');
  this.stake = br.readU8() === 1;
  return this;
};
