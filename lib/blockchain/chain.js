...
const Pos = require('../protocol/pos');
...
Chain.prototype.verifyInputs = async function verifyInputs(block, prev, state) {
  ...
  for (let i = 0; i < block.txs.length; i++) {
    ...
    if (i > 0) {
      ...
      if (tx.isCoinstake())
        stakeReward = tx.getOutputValue() - tx.getInputValue(view);
      else
        reward += fee;
  ...
  if(block.stake){
    reward += consensus.getStakeReward();
    if (stakeReward > reward) {
      throw new VerifyError(block,
        'invalid',
        'bad-cs-amount',
        100);
...
Chain.prototype.connect = async function connect(prev, block, flags) {
  ...
  if(!this.isGenesis(block)){
    entry.stakeModifier = this.pos.computeStakeModifier(prev.stakeModifier, block.stake ?
      block.txs[1].inputs[0].prevout.hash :
      entry.hash);
  }
  ...
  if(!block.stake){
    ...
    let previousout = block.txs[1].inputs[0].prevout;
    let coin = await this.db.getCoins(previousout.hash);
    let blkbits = block.bits;
    let timeTx = block.txs[1].txTime;

    if (!this.pos.checkStakeKernelHash(prev, blkbits, coin, previousout, timeTx))
      throw new VerifyError(block, 'wrong proof of stake', 50, true);
  }
...
Chain.prototype.getTarget = async function getTarget(ts, prev) {
  ...
  // Back 1 block, retargetInterval = 2
...
Chain.prototype.getTargetLimit = function getTargetLimit(ts, pos){
  var nLimit;
  if(pos) {
    if (consensus.isProtocolV2(ts))
      nLimit = this.network.pow.posLimit2;
    else
      nLimit = this.network.pow.posLimit;
  }else {
    nLimit = this.network.pow.limit;
  }
  return nLimit;
...
Chain.prototype.retarget = function retarget(prev, first) {
  let pow = this.network.pow;
  let actualSpacing = prev.ts - first.ts;
  let targetSpacing = this.network.isProtocolV2(prev.ts)? pow.targetSpacing :
    pow.targetSpacingV1;

  // Limit adjustment step
  if (prev.ts > pow.protocolV1RetargetingFixedTime && actualSpacing < 0)
    actualSpacing = targetSpacing;
  if (prev.ts > pow.protocolV3Time && actualSpacing > targetSpacing*10)
    actualSpacing = targetSpacing*10;

  if (pow.noRetargeting)
    return prev.bits;

  let targetLimit = this.getTargetLimit(prev.ts, prev.stake);
  var bNew = consensus.fromCompact(prev.bits);
  let interval = pow.targetTimespan / targetSpacing;

  let multiplier = new BN((interval - 1) * targetSpacing + actualSpacing + actualSpacing);
  let divider = new BN((interval + 1) * targetSpacing);
  bNew = bNew.imul(multiplier);
  bNew = bNew.idivn(divider);

  if (bNew.lten(0) || bNew.gt(targetLimit))
    bNew = targetLimit;

  return consensus.toCompact(bNew);
};
...
Chain.prototype.getDeploymentState = async function getDeploymentState() {
  ...
  if (!prev) {
    this.logger.info(this.network.genesis.hash.toString());
    this.logger.info(this.tip.hash.toString());
