 ...
 */
Script.prototype.getPubKey = function getPubKey() {
  if (this.code.length != 2)
    throw new ScriptError('Script not of right size, expecting 2');

  const chunk0data = this.code[0];
  const chunk1data = this.code[1];
  if (chunk0data.data && chunk0data.data.length > 2 && chunk1data.data && chunk1data.data.length != 0) {
    // If we have two large constants assume the input to a pay-to-address output.
    return chunk1data.data;
  } else if (chunk0data.value === opcodes.OP_RETURN && chunk1data.data && chunk1data.data.length != 0) {
    // A large constant followed by an OP_CHECKSIG is the key.
    return chunk1data.data;
  } else if (chunk1data.value === opcodes.OP_CHECKSIG && chunk0data.data && chunk0data.data.length != 0) {
    // A large constant followed by an OP_CHECKSIG is the key.
    return chunk0data.data;
  } else {
    throw new ScriptError('Script did not match expected form');
  }
};
